...
@app.route("/")
def create_excel_file(jobid, employee, jobdate, reports, file_name):
    cell_names = ['C1', 'D1', 'E1', 'F1']
    eval_names = ["Travel In","Team Arrival","Time In","Time Out"]

    outWorkbook = xlsxwriter.Workbook(os.path.join(os.path.expanduser('~'), file_name))
    row = 1
